...
__global__ void convert(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray)
{
    ...
    // int threadId = blockId * (gridDim.x * gridDim.y * gridDim.z) + threadIdx.z * (blockDim.x * blockDim.y) + threadIdx.y * blockDim.x + threadIdx.x;

    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int threadId = y * d_rows + x;

    if(threadId < d_rows*d_columns){
        //d_gray[threadId] = (d_r[threadId] + d_g[threadId] + d_b[threadId]) / 3;
        d_gray[threadId] = static_cast<uchar>(0.299f * d_r[threadId] + 0.587f * d_g[threadId] + 0.114f * d_b[threadId]);
    }
}
...
__host__ void executeKernel(uchar *d_r, uchar *d_g, uchar *d_b, uchar *d_gray, int rows, int columns, int threadsPerBlock)
{
    ...
    int gridCols = (columns + threadsPerBlock - 1) / threadsPerBlock; // max(columns/(threadsPerBlock*blockZSize),1);
