...
__host__ float * runStreamsFullAsync(float *host_mem, int num_elements)
{
    ...
    // int s;
    // std::cout << "Enter a seed value (0-255) for A1 (runStreamsFullAsync): ";
    // std::cin >> s;
    srand(42);
    float x = static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/random_max));

    // Ensure user input has been received before proceeding
    cudaStreamSynchronize(stream2);

    // Execute 4 kernels asynchronously on independent streams
    auto[blocksPerGrid, threadsPerBlock] = determineThreadBlockDimensions(num_elements);
    ...
    // std::cout << "Enter a seed value (0-255) for A2 (runStreamsFullAsync): ";
    // std::cin >> s;
    srand(42);
    ...
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, stream4>>>(dev_mem, num_elements, x);
...
__host__ float * runStreamsBlockingKernel2StreamsNaive(float *host_mem, int num_elements)
{
    ...
    kernelA2<<<blocksPerGrid, threadsPerBlock, 0, stream3>>>(dev_mem, num_elements, x);
